...
+ (SecKeyRef)addPublicKey:(NSString *)key{
    ...
    NSData *d_tag = [NSData dataWithBytes:[tag UTF8String]
                                   length:[tag length]];
    
    // Delete any old lingering key with the same tag
    NSMutableDictionary *publicKey = [[NSMutableDictionary alloc] init];
    [publicKey setObject:(__bridge id)kSecClassKey
                  forKey:(__bridge id)kSecClass];
    [publicKey setObject:(__bridge id)kSecAttrKeyTypeRSA
    ...
    OSStatus status = SecItemAdd((__bridge CFDictionaryRef)publicKey,
                                 &persistKey);
    if (!persistKey){
    ...
    status = SecItemCopyMatching((__bridge CFDictionaryRef)publicKey,
                                 (CFTypeRef *)&keyRef);
...
+ (SecKeyRef)addPrivateKey:(NSString *)key{
    ...
    [privateKey setObject:(__bridge id)kSecClassKey
                   forKey:(__bridge id)kSecClass];
    [privateKey setObject:(__bridge id)kSecAttrKeyTypeRSA
                   forKey:(__bridge id)kSecAttrKeyType];
    ...
    // Now fetch the SecKeyRef version of the key
    SecKeyRef keyRef = nil;
    status = SecItemCopyMatching((__bridge CFDictionaryRef)privateKey,
                                 (CFTypeRef *)&keyRef);
    if(status != noErr) return nil;
    return keyRef;
}
...
+ (NSData *)encryptData:(NSData *)data
    ...
    return [RSAUtil encryptData:data
                     withKeyRef:keyRef];
}
...
+ (NSData *)decryptData:(NSData *)data privateKey:(NSString *)privKey{
    ...
    SecKeyRef keyRef = [RSAUtil addPrivateKey:privKey];
    if(!keyRef) return nil;
    return [RSAUtil decryptData:data
                     withKeyRef:keyRef];
}
