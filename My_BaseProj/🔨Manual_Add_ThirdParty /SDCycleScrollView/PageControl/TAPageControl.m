...
- (void)touchesBegan:(NSSet *)touches
    ...
    if (touch.view != self) {
        ...
        if ([self.delegate respondsToSelector:@selector(TAPageControl:didSelectPageAtIndex:)]) {
            ...
            [self.delegate TAPageControl:self
                    didSelectPageAtIndex:index];
        }
    }
}
...
- (void)updateFrame:(BOOL)overrideExistingFrame{
    ...
    if (overrideExistingFrame ||
        ((CGRectGetWidth(self.frame) < requiredSize.width ||
          CGRectGetHeight(self.frame) < requiredSize.height) && !
         overrideExistingFrame)) {
 ...
 */
- (void)updateDotFrame:(UIView *)dot
...
- (void)setCurrentPage:(NSInteger)currentPage{
    ...
    // Pre set
    [self changeActivity:NO
                 atIndex:_currentPage];
    _currentPage = currentPage;
    
    // Post set
    [self changeActivity:YES
                 atIndex:_currentPage];
}
...
- (CGSize)dotSize{
    ...
    if (self.dotImage &&
        CGSizeEqualToSize(_dotSize, CGSizeZero)) {
